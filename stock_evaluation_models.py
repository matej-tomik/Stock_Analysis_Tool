...
dcf is according to https://www.gurufocus.com/term/iv_dcf/AAPL/Intrinsic-Value:-DCF-(FCF-Based)/Apple

ddm is according to https://finbox.com/NASDAQGS:AAPL/models/ddm-sg/

graham_number it simply according to google
"""""


def dcf(earnings_per_share: float, wacc: float, growth_rate: float, terminal_growth_rate: float = 0.04, year: int = 10) -> float:
    if growth_rate < 0.05:
        growth_rate = 0.05
    x = 0
...
def ddm(dividend: float, shares_outstanding: int, net_income: int, dividend_growth_rate: float, capm: float, growth_rate: float) -> float:
...
def graham_number(earnings_per_share: float, book_value_per_share: float) -> float:
    ...
    return sqrt(result) if result > 0 else -1
